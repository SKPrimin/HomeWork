...
/* Initialize the shell variables from the current environment.
   ...
   parse $SHELLOPTS. */
void
    initialize_shell_variables(env, privmode) char **env;
int privmode;
{
...
void
    adjust_shell_level(change) int change;
{
...
void
    sh_set_lines_and_columns(lines, cols) int lines,
    cols;
{
...
void
    print_var_list(list) register SHELL_VAR **list;
{
  register int i;
...
/* Print LIST (a list of shell functions) to stdout in such a way that
   they can be read back in. */
void
    print_func_list(list) register SHELL_VAR **list;
{
  register int i;
  register SHELL_VAR *var;
...
static SHELL_VAR *
    init_dynamic_array_var(name, getfunc, setfunc, attrs) char *name;
...
static SHELL_VAR *
    init_dynamic_assoc_var(name, getfunc, setfunc, attrs) char *name;
sh_var_value_func_t *getfunc;
sh_var_assign_func_t *setfunc;
int attrs;
{
...
static void
    sbrand(seed) unsigned long seed;
{
  rseed = seed;
  last_random_value = 0;
}
...
void
    make_funcname_visible(on_or_off) int on_or_off;
{
  SHELL_VAR *v;
...
SHELL_VAR *
    set_if_not(name, value) char *name,
    *value;
{
  SHELL_VAR *v;
...
#if defined(ARRAY_VARS)
SHELL_VAR *
    make_new_array_variable(name) char *name;
{
  SHELL_VAR *entry;
  ARRAY *array;
...
SHELL_VAR *
    make_local_array_variable(name) char *name;
{
  SHELL_VAR *var;
  ARRAY *array;
...
SHELL_VAR *
    make_new_assoc_variable(name) char *name;
{
  SHELL_VAR *entry;
  HASH_TABLE *hash;
...
SHELL_VAR *
    make_local_assoc_variable(name) char *name;
{
  SHELL_VAR *var;
  HASH_TABLE *hash;
...
SHELL_VAR *
    bind_int_variable(lhs, rhs) char *lhs,
    *rhs;
{
  register SHELL_VAR *v;
  int isint, isarr;
...
SHELL_VAR *
    bind_var_to_int(var, val) char *var;
intmax_t val;
{
...
void
    set_var_read_only(name) char *name;
{
  SHELL_VAR *entry;
...
/* Make the variable associated with NAME be auto-exported.
   If NAME does not exist yet, create it. */
void
    set_var_auto_export(name) char *name;
{
  SHELL_VAR *entry;
...
static VARLIST *
    vlist_alloc(nentries) int nentries;
{
  VARLIST *vlist;
...
char **
    add_or_supercede_exported_var(assign, do_alloc) char *assign;
int do_alloc;
{
  register int i;
  int equal_offset;
...
static void
    add_temp_array_to_env(temp_array, do_alloc, do_supercede) char **temp_array;
...
void
    update_export_env_inplace(env_prefix, preflen, value) char *env_prefix;
int preflen;
char *value;
{
  char *evar;
...
void
    put_command_name_into_env(command_name) char *command_name;
{
...
VAR_CONTEXT *
    new_var_context(name, flags) char *name;
int flags;
{
  VAR_CONTEXT *vc;
...
/* Make a new variable context with NAME and FLAGS and a HASH_TABLE of
   ...
   for shell functions. */
VAR_CONTEXT *
    push_var_context(name, flags, tempvars) char *name;
int flags;
HASH_TABLE *tempvars;
{
  VAR_CONTEXT *vc;
...
VAR_CONTEXT *
    push_scope(flags, tmpvars) int flags;
HASH_TABLE *tmpvars;
{
...
void
    pop_scope(is_special) int is_special;
{
...
void
    push_context(name, is_subshell, tempvars) char *name; /* function name */
int is_subshell;
HASH_TABLE *tempvars;
{
  if (is_subshell == 0)
...
void push_dollar_vars()
{
  if (dollar_arg_stack_index + 2 > dollar_arg_stack_slots)
  {
    dollar_arg_stack = (WORD_LIST **)
        xrealloc(dollar_arg_stack, (dollar_arg_stack_slots += 10) * sizeof(WORD_LIST **));
  }
...
static int
    sv_compare(sv1, sv2) struct name_and_function *sv1,
    *sv2;
{
  int r;
...
void
    stupidly_hack_special_variables(name) char *name;
{
...
#if defined(READLINE)
void
    sv_comp_wordbreaks(name) char *name;
{
  SHELL_VAR *sv;
...
void
    sv_history_control(name) char *name;
{
  char *temp;
  char *val;
  int tptr;
...
void
    sv_strict_posix(name) char *name;
{
...
#if defined(ARRAY_VARS)
void
    set_pipestatus_array(ps, nproc) int *ps;
int nproc;
{
  SHELL_VAR *v;
  ARRAY *a;
...
void
    set_pipestatus_from_exit(s) int s;
{
#if defined(ARRAY_VARS)
  static int v[2] = {0, -1};
