...
p1(T+1)=psnr(stegoi1,coverRGB);
fprintf('嵌入第一层水印后的载密图像与载体图像的psnr值：%f\n',p1);
%% EMD算法在RGB域嵌入第二层水印
...
p2(T+1)=psnr(stegoi2,coverRGB);
fprintf('嵌入第二层水印后的载密图像与载体图像的psnr值：%f\n',p2);
%% 提取第二层水印
...
title('提取到的第二层水印图像');
p3(T+1)=psnr(bin_exdata2,ori_water2);
fprintf('提取到的第二层水印图像与原始水印图像的psnr值：%f\n',p3);
%% 提取第一层水印
...
title('提取到的第一层水印图像');
p4(T+1)=psnr(uint8(dec_exdata1),ori_water1);
fprintf('提取到的第一层水印图像与原始水印图像的psnr值：%f\n',p4);